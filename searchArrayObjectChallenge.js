...
const todos0 = todos.map(item => item = {title: item, status: 'done', location: 'home'})
//console.log(todos0)
...
const deleteTodo = function (todos, key) {
    let toDeleteIndex = todos.findIndex(item => item.title.toLowerCase() === key.toLowerCase())
    let toDelete = todos.find(item => item.title.toLowerCase() === key.toLowerCase())
    if (toDeleteIndex > -1) {
...
const notes = [{title: 'My Next Trip', body: 'I would like to go to Spain'}, {title: 'Habbits to work on', body: 'More Excercise'}, {title: 'Office Modification', body: 'Ergonomic Keyboard'}]
...
const findNotes = (array, key) => {
    const filteredNotes = array.filter(item => item.title.toLowerCase().includes(key) || item.body.toLowerCase().includes(key))
    return console.log(filteredNotes)
}
...
const findTodoNext = function (array, key) {
    const filteredTodoNext = array.filter(item => item.status.toLowerCase().includes(key))
